...
class ScalaJSTestNGPlugin(val global: Global) extends NscPlugin {
  ...
  private lazy val scalaJSPlugin = {
    ...
    global.plugins.collectFirst {
      ...
      throw new Exception("The Scala.js TestNG plugin only works with the Scala.js plugin enabled.")
    }
  }

  object ScalaJSTestNGPluginComponent extends plugins.PluginComponent with transform.Transform with Compat210Component {
    ...
    class ScalaJSTestNGPluginTransformer extends Transformer {
      ...
      private val testNGClassMetadataType =
        getRequiredClass("io.github.soc.testng.TestNGClassMetadata").toType

      private val testNGTestMetadataType =
        getRequiredClass("io.github.soc.testng.TestNGTestBootstrapper").toType

      private def testNGMethodMetadataTypeTree =
        TypeTree(getRequiredClass("io.github.soc.testng.TestNGMethodMetadata").toType)
      ...
      def mkBootstrapperClass(clazz: ClassDef, modDefOption: Option[ClassDef]): ClassDef = {
        ...
        val getTestNGMetadataDef = mkGetTestNGMetadataDef(clazz.symbol, modDefOption.map(_.symbol))
        ...
        val invokeTestNGMethodDef = {
          ...
          mkInvokeTestNGMethodOnModuleDef(annotatedMethods, bootSym, modDefOption.map(_.symbol))
        }
        val invokeTestNGMethodOnInstanceDef = {
          val annotatedMethods = testNGAnnotatedMethods(clazz.symbol.asClass)
          mkInvokeTestNGMethodOnInstanceDef(annotatedMethods, bootSym, clazz.symbol)
        }

        val bootBody =
          List(getTestNGMetadataDef, newInstanceDef, invokeTestNGMethodDef, invokeTestNGMethodOnInstanceDef)
        ...
        val newClazzInfo = {
          val newParentsInfo = List(definitions.ObjectTpe, testNGTestMetadataType)
      ...
      def testNGAnnotatedMethods(sym: Symbol): List[MethodSymbol] = {
        sym.selfType.members.collect {
          case m: MethodSymbol if hasTestNGTestMethodAnnotation(m) => m
        }.toList
      }
       ...
       *  }}}
       */
      def mkInvokeTestNGMethodOnModuleDef(
        ...
        val invokeTestNGMethodRhs = mkMethodResolutionAndCall(invokeTestNGMethodSym, methods, paramSyms.head, callLocally)
       ...
       *  }}}
       */
      def mkInvokeTestNGMethodOnInstanceDef(methods: List[MethodSymbol], classSym: Symbol, refClassSym: Symbol): DefDef = {
        val invokeTestNGMethodSym = classSym.newMethod(newTermName("invoke"))

        val paramSyms = {
          val params = List(("instance", definitions.ObjectTpe), ("methodName", definitions.StringTpe))
          mkParamSymbols(invokeTestNGMethodSym, params)
        }
        ...
        val invokeTestNGMethodRhs = mkMethodResolutionAndCall(invokeTestNGMethodSym, methods, idParamSym, callLocally)

        mkMethod(invokeTestNGMethodSym, invokeTestNGMethodRhs, paramSyms)
      }

      def mkGetTestNGMetadataDef(clSym: Symbol, modSymOption: Option[Symbol]): DefDef = {
        ...
        def defaultMethodMetadata(tpe: TypeTree)(mtdSym: MethodSymbol): Tree = {
          ...
          mkNewInstance(tpe, List(Literal(Constant(mtdSym.name.toString)), mkList(annotations)))
        }
      ...
      private def hasTestNGTestMethodAnnotation(mtd: MethodSymbol): Boolean =
        annotationWhiteList.filterNot(_ == DataProviderClass).exists(hasAnnotation(mtd, _))
      ...
      private def mkParamSymbols(method: MethodSymbol, params: List[(String, Type)]): List[Symbol] = {
        params.map {
          case (pName, tpe) =>
      ...
      private def mkMethod(methodSym: MethodSymbol, methodRhs: Tree, paramSymbols: List[Symbol]): DefDef = {
      ...
      private def mkMethodResolutionAndCall(methodSym: MethodSymbol, methods: List[Symbol], idParamSym: Symbol, genCall: Symbol => Tree): Tree = {
