...
final class TestNGExecuteTest(
    taskDef: TaskDef,
    runner: TestNGBaseRunner,
    classMetadata: TestNGTestBootstrapper,
  ...
  private[this] def executeTestMethod(classMetadata: TestNGTestBootstrapper, method: TestNGMethodMetadata) = {
    val testNGMetadata = classMetadata.metadata()
    ...
    def executeTestMethods(): Unit = {
      ...
      try {
        ...
        logFormattedDebug(decodedMethodName, s"finished, took ${getTimeInSeconds()} sec")
    ...
    def executeAfterMethods(): Unit = {
      val timeInSeconds = getTimeInSeconds().toLong
      try {
        ...
        case ex: Throwable =>
          ...
          eventHandler.handle(new TestNGEvent(taskDef, Status.Failure, selector, NoThrowable, timeInSeconds))
      }
    }
    ...
    try {
      ...
      // println(method.getDataProviderName)
      executeTestMethods()
    } catch {
  ...
  private def ignoreTest(methodName: String) = {
    runner.testIgnored()
    val selector = new NestedTestSelector(fullyQualifiedName, methodName)
    eventHandler.handle(new TestNGEvent(taskDef, Status.Ignored, selector))
  }
  ...
  private[this] def logAssertionWarning(methodName: String, ex: Throwable,
    ...
    val msg = s"failed: org.junit.internal.${c("AssumptionViolatedException", ERRMSG)}: ${ex.getMessage}, took $timeInSeconds sec"
  ...
  private[this] def logFormattedWarn(prefix: String, method: String, msg: String): Unit = {
    val fMethod = if (method != null) c(method, ERRMSG) else null
    richLogger.warn(
        formatLayout(prefix, packageName, c(className, NNAME1), fMethod, msg))
  }

  private[this] def logFormattedError(method: String, msg: String, exOpt: Option[Throwable]): Unit = {
    val fMethod = if (method != null) c(method, ERRMSG) else null
