...
import io.github.soc.testng.TestNGFramework
...
abstract class TestNGTest {
  ...
  private val classLoader: ClassLoader = TestNGTestPlatformImpl.getClassLoader
  ...
  @Test def testTestNGOutput(): Unit = {
    for (frameworkArgs <- frameworkArgss) {
      ...
      val recorder: TestNGTestRecorder = new TestNGTestRecorder(new OutputContext(frameworkArgs))
  ...
  protected class OutputContext(frameworkArgs: List[String]) {
    ...
    def testStartedOutput(method: String): Output =
      infoIfOrElseDebug(verbose, s"Test $formattedTestClass.${cyan(method)} started")
    ...
    def testRunStartedOutput: Output =
      // TODO: Figure out why we needed to disable this, but nothing else ...
    ...
    def testRunFinishedOutput: Output = {
      ...
      buff.append(blue(", "))
      val skippedColor = if (expectedSkipped == 0) blue _ else yellow _
      buff.append(skippedColor(s"$expectedSkipped skipped"))
      buff.append(blue(s", $expectedTotal total, ${TIME_TAG}s"))
      infoIfOrElseDebug(verbose, buff.result())
    }

    def ignoredEvent: Output = Event("Ignored")
  ...
  protected class TestNGTestRecorder(context: OutputContext) extends Logger with EventHandler {
    ...
    def error(msg: String): Unit =
      if (msg.startsWith("    at ") || msg.startsWith("    ...")) {
        //println(GREY + "Ignoring stacktrace output: " + withoutColor(msg) + NORMAL) // TODO: figure out why this gets printed everywhere ...
      } else {
    ...
    def handle(event: sbt.testing.Event): Unit = buff += Event(event.status().toString)
    ...
    def checkOutput(): Unit = {
      ...
      val (expected1, actual1, droppedFront) = minimizeDiff(expected, actual, 0)
      val (expected2, actual2, droppedBack) = minimizeDiff(expected1.reverse, actual1.reverse, 0)
